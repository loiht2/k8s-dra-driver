...
func (l deviceLib) GetPerGpuAllocatableDevices(indices ...int) (PerGPUMinorAllocatableDevices, error) {
	...
	err := l.VisitDevices(func(i int, d nvdev.Device) error {
		...
		if featuregates.Enabled(featuregates.HAMiCoreSupport) {
			// Use the HAMi canonical name ("hami-gpu-N") so that allocations
			// referencing the device by that name resolve in s.allocatable
			// during Prepare(). Using gpuInfo.CanonicalName() ("gpu-N") would
			// publish the device under "hami-gpu-N" via GetDevice() but key
			// the allocatable map under "gpu-N", causing a Prepare() mismatch.
